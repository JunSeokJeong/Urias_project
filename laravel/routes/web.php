...
Route::get('/resultprocess/{id}','StudyController@quizResultProcess');
Route::get('/result/{id}','StudyController@quizResult');//사용자의 퀴즈최종결과 반환 
...
Route::get('/library/volunteerInputText2/{num}/{book_name}/{page}/{count}',  'VolunteerController@volunteerInputText2')->name('vInputText');//오역수정페이지로 이동
Route::post('/library/volunteerInputText',                                   'VolunteerController@volunteerInputTextSubmit');              //오역수정페이지로 이동
Route::post('/library/volunteerInputText2',                                  'VolunteerController@volunteerInputTextSubmit2');              //오역수정페이지로 이동
Route::get('/library/volunteerCheck/{num}/{book_name}/{page}',               'VolunteerController@volunteerCheck')->name('vCheck');        //오역수정검수 페이지로 이동
Route::get('/library/volunteerCheck2/{num}/{book_name}/{page}',              'VolunteerController@volunteerCheck2')->name('vCheck');        //오역수정검수 페이지로 이동
Route::post('/library/volunteerCheck',                                       'VolunteerController@volunteerCheckRegister');        //
Route::post('/library/volunteerCheck2',                                      'VolunteerController@volunteerCheckRegister2');        //
...
Route::get('/blindcare/volunteerTimeList/{year}/{month}',      'VideoCallController@volunteerTimeList')->name('vTList');             // 봉사자 봉사시간 조회 페이지로 이동(시각장애인)
...
Route::get('/blindcare/fallingLocationApp',                    'VideoCallController@fallingLocationApp')->name('fLocationApp');      // 낙상사고 리스트 로드(앱, 시각장애인)
Route::get('/blindcare/rtcLoadApp',                            'VideoCallController@rtcLoadApp')->name('rLoadApp');                  // 봉사자의 웹rtc 정보 로드(앱, 시각장애인)
Route::get('/blindcare/rtcReady',                               'VideoCallController@rtcReady')->name('rtcReady');         // 웹rtc 대기화면
Route::get('/blindcare/rtcChat',                                'VideoCallController@rtcChat')->name('rtcChat');         // 웹rtc 영상화면
...
Route::get('/shop/popup/{id}',                                            'ShopController@popup')->name('popup/{id}');
Route::post('/shop/buy',                                            'ShopController@buy')->name('buy');
Route::get('/shop/Basketpopup/{id}',                                            'ShopController@Basketpopup')->name('Basketpopup/{id}');
